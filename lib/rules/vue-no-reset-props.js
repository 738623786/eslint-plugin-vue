/**
 * @fileoverview disallow reset component props
 * @author 2018 Armano
 */
'use strict'
 ...
 * @param { boolean } options.shallowOnly Enables reset the value of a prop but leaving the reference the same
 */
...
module.exports = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'disallow reset of component props',
      categories: ['vue3-essential', 'essential'],
      url: 'https://eslint.vuejs.org/rules/vue-no-reset-props.html'
    },
    fixable: null,
    schema: [
      {
    ...
    messages: {
      unexpectedReset: 'Unexpected reset of "{{key}}" prop.'
    }
  },
  ...
  create(context) {
     ...
     */
    function verifyReset(props, name, isRootProps = false) {
      const invalid = utils.findMutating(props, true)
      ...
      utils.defineTemplateBodyVisitor(context, {
        ...
        'VExpressionContainer Identifier'(node) {
          ...
          if (name && (propsInfo.set.has(name) || isRootProps)) {
            verifyReset(node, name, isRootProps)
          }
        },
        /** @param {ESNode} node */
